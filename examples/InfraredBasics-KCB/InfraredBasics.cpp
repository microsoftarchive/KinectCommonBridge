...
CInfraredBasics::~CInfraredBasics()
{
    ...
    // delete the buffer
    SafeArrayDelete(m_kcbInfraredFrame.Buffer);
...
void CInfraredBasics::Update()
{
    ...
    HRESULT hr = KCBGetInfraredFrame(m_hKCB, &m_kcbInfraredFrame);
    ...
    if (SUCCEEDED(hr))
    {
        ...
        if (SUCCEEDED(hr))
        {
            ProcessInfrared(m_kcbInfraredFrame.TimeStamp, m_kcbInfraredFrame.Buffer, cInfraredWidth, cInfraredHeight);
        }
...
HRESULT CInfraredBasics::InitializeDefaultSensor()
{
    ...
    if (SUCCEEDED(hr))
    {
        hr = KCBGetInfraredFrameDescription(m_hKCB, &frameDesc);
        if (SUCCEEDED(hr))
        {
            m_kcbInfraredFrame.Size = frameDesc.lengthInPixels;
            m_kcbInfraredFrame.Buffer = new UINT16[m_kcbInfraredFrame.Size];
        }
    }
...
bool CInfraredBasics::SetStatusMessage(WCHAR* szMessage, DWORD nShowTimeMsec, bool bForce)
{
...
/// </returns>
HRESULT CInfraredBasics::GetScreenshotFileName(LPWSTR lpszFilePath, UINT nFilePathSize)
{
